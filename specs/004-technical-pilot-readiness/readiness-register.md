...
|---|---|---|---|
| Host allocation and network path | [environment-profile.md](environment-profile.md), Sections 1–2 | Authenticated SSH inventory confirms Ubuntu 26.04.1 LTS, current DHCP route, resources and dedicated artifact mount; application endpoints and stable client route remain `NOT-RUN` | D2 remains `OPEN`; P04/P05 and P06 documentary results are `PASS`; runtime checks and PG4 remain separate |
...
checks remain `NOT-RUN`. D0 remains `OPEN` for its formal exact-successor/PH1 scope disposition
(with its architecture direction confirmed). D3 is satisfied for the current documentary scope and
reopens for the deferred recovery/storage claims. Other D1–D5 dispositions remain open where their
evidence is missing.
...
| P04, P05 and P06 result records themselves | `CONTRACT-REQUIRES-NOW` | Yes, under current PH0 tasks/PG4 path | — | T022 is complete: each package has a separate result, exact evidence identity and reviewer record. A later-scope check may remain `NOT-RUN` under its documented boundary. |
