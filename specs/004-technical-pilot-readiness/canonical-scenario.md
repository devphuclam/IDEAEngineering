...
|---|---|---|
| Design Engineer | Uses a native IDEA account, selects Workspace scope, edits a materialized file and submits a Check-in. | Cannot approve or Release the same Revision under the seeded independent-approver rule. |
| Independent Approver | Reviews the exact submitted Generation and records an attributable decision. | Must be a separately eligible identity; two accounts operated by one person are not independent-human evidence. |
...
5. A synthetic changed Artifact, an unchanged Artifact and a deliberately stale candidate are
   available. The final size, multi-GB fixture provenance and Vault failure domains remain
   `UNKNOWN`/`BLOCKED` until P03/P05 close them.
...
| 6 | Scan the complete proposed scope and confirm the Check-in set. | The user sees `Unchanged`, `Modified under Checkout`, `Modified without Checkout`, `Missing`, `Out of date` and `Unresolved` states; an unreserved changed dependency is excluded or blocks according to policy. | `REQ-WS-005/006`; `VVP-002`, `WS-02` |
...
| 10 | Independent Approver records approval or rejection with a reason. | Self-approval is refused; the decision records actor, time, exact Generation/scope, policy version and reason. | `REQ-LC-003/004/005`; `VVP-006`, `WF-04…06` |
| 11 | Confirm one exact Release scope and commit Release. | Commit-time revalidation covers each required Generation, Structure Snapshot, access and exception; one immutable Release Record and exact package are retained, with no silent dependency cascade. | `REQ-LC-006/007/008`, `REQ-STR-001…005`; `VVP-006`, `SR-01…06` |
...
| Unreserved changed dependency | A related document changed without a Reservation. | Exclude it or block the confirmed operation according to the approved policy; never publish silently. | Preserve the candidate and show the reason. | `REQ-WS-005/006`; `VVP-002`, `WS-02` |
