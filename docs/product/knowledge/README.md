...
| Technology decision | `IE-KNW-TECH-DEC-001@0.7`: Linux-first Java/Temurin/Spring/PostgreSQL Server and React/WPF/WebView2 + per-user `.NET Workspace` are the selected Core v0 Engineering baseline; Node.js 24 LTS is the Product Decision Authority-approved Web-build family while the exact build remains `NOT-RUN`; Flutter remains an evaluated alternative, Q-01…Q-14 remain `NOT-RUN`, Q-15 remains `PARTIAL / NO WINNER`, and no product gate becomes PASS |
