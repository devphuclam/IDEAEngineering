...
The [Vault successor gallery](evidence/IE-VEV-VAULT-XFER-002/index.html) remains the historical
full-gallery rendition of its pinned DOC-05@0.21/DOC-06@0.17 baseline, including the control/data
plane and multi-location Vault views. Its provenance is recorded in
...
predecessor. Version 0.3 added the future multi-location Vault architecture; version 0.4 recorded Node.js 24
...
adds working Saturdays in weeks 1, 3 and 5 of each month (weeks 2 and 4 are days off),
giving 79 baseline working days / 632 hours before separately recorded holidays and other assignments. The 31 December milestone is a bounded production-shaped thread, not full Core
v0, production readiness or company rollout. PH0 has recorded results through P06 in the
working Execution Register; product implementation and `PG4` remain `NOT-RUN`.
...
- [Kanban CARIO@0.8 — 53 delivery cards, 7 zero-effort decision/milestone cards and P07 review substeps](planning/idea-technical-pilot-kanban-cario.md)
...
| [`TECH-001`](decision-briefs/TECH-001-technology-and-architecture-proposal.md) | Architecture and technology selection | DOC-02, DOC-04/05/06/08, accepted ADRs, confirmed context, [`IE-KNW-TECH-DEC-001`](../../knowledge/2026-09-13-core-v0-technology-decision-matrix.md), technology views and official sources | `Draft 0.16` is the current successor brief. Node.js 24 LTS is `APPROVED` for Web build; exact Web build remains `NOT-RUN`. The reviewed Tech baseline `0.14` remains `APPROVED`; authority for other successor deltas follows their separate records. Q-15 remains `PARTIAL / NO WINNER`, exact Format Worker and Artifact Gateway runtime/toolchain remain `NOT-RUN`, and PG3 remains `NOT-RUN`. |
...
| DOC-07@0.5 | Historical source-reconciliation baseline; superseded through current `DOC-07@0.17`. The old 56-task/756-hour schedule remains recoverable in Git history; it is replaced for execution planning by the 35-work-package Core v0 baseline selected on 21-09-2026 and identity-corrected on 22-09-2026. |
...
| `VVP` | `IE-VVP-CORE-001` | [Core v0 Verification and Validation Plan](registers/VVP-core-v0-verification-validation-plan.md) | `Draft 0.19`; 17 objectives plus PA-01…04, RBAC-01…10, WS-01…11 and ST-01…07; the Check-in scope and other product procedures remain `NOT-RUN` |
...
| `VEV` | `IE-VEV-ARCH-CORR-005` | [Module Authority View Legibility Correction](registers/VEV-2026-09-14-module-authority-view-legibility.md) | `Draft 0.1`; predecessor 30-view source/rendition baseline for DOC-05@0.20 and DOC-06@0.16; successor source/rendition evidence is `IE-VEV-VAULT-XFER-002`; qualified review and controlled-rendition acceptance remain `BLOCKED` |
...
| `CHG` | `IE-CHG-TECH-NODE24-001` | [Node.js 24 Web-Build Baseline](registers/CHG-2026-09-23-node24-web-build-baseline.md) | `Draft 0.1`; records the PDA-approved Node.js 24 LTS Web-build-family decision separately from document status; exact package graph/build remains `NOT-RUN`; retained Q-15 evidence, Product Scope, PG3 and PG4 unchanged |
| `CHG` | `IE-CHG-PH0-CORR-002` | [Post-analysis Consistency Correction](registers/CHG-2026-09-23-post-analysis-consistency-correction.md) | `Draft 0.1`; aligns Core v0 one-Vault diagrams, P01 evidence, current planning references and tracker-test isolation; no Feature, Spec, Tech, Product Scope, Q-15, PDA, PG3 or PG4 change |
...
| `CHG` | `IE-CHG-PDA-APPROVAL-003` | [PDA Approval — Check-in Scope](registers/CHG-2026-09-25-pda-approval-checkin-scope.md) | `Draft 0.1` record; Product Decision Authority approved all three Check-in scope branches on 25-09-2026; architecture review and runtime verification unchanged |
...
| `CHG` | `IE-CHG-PLAN-ID-001` | [Core v0 Planning Card Identity Correction](registers/CHG-2026-09-22-planning-card-identity-correction.md) | `Current 0.1`; replaces the first three completed management-planning cards with `PLN01`–`PLN03`, records 12 retrospective planning hours and preserves 512 hours, scope, milestones, Product Decision Authority decisions and PG3/PG4 states |
...
[`TECH-001@0.16`](decision-briefs/TECH-001-technology-and-architecture-proposal.md). The matrix did
