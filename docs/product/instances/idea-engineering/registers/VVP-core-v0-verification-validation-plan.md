...
| `VVP-003` — `REQ-WS-007`…`REQ-WS-012`, `REQ-WS-015`, `QRS-001/002/011` | Fault injection at chunk upload, candidate verification, preflight, database commit, response and reconciliation; OperationId status/replay. | Exact future adapters/stores with controllable faults and multi-document/multi-GB fixtures. | Zero partial public Change Sets/Generations, no prematurely ended Reservation, preserved local files, resumed verified ranges and one idempotent terminal result per OperationId. | Verification Authority `BLOCKED` | `NOT-RUN` |
