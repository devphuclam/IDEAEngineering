...
> **Instance state**: controlled `Draft 0.18`. Sections 1–7 define the information and exchange
...
| Change History | 0.18: retain the pinned Approval Policy Version, `AllowSelfApproval` outcome and independent-approval evaluation in the decision data contract; [IE-CHG-APPROVAL-POLICY-001](registers/CHG-2026-09-19-approval-policy-self-approval.md). 0.17: add Vault Endpoint, scoped Transfer Grant, authenticated Transfer Receipt, one-to-many Artifact locations, replication tasks and versioned durability policy; replace server-proxied bytes with a direct Gateway data plane while retaining Server publication authority; [IE-CHG-VAULT-XFER-001](registers/CHG-2026-09-17-multi-location-vault-transfer-architecture.md). 0.16: clarify owner-specific ArtifactId/digest pins and BOM export acceptance; [IE-CHG-ARCH-CORR-003](registers/CHG-2026-09-12-architecture-consistency-correction-003.md). Earlier history remains in controlled change records. |
...
| Generation Manifest | Immutable published Product Definition: revision/version coordinate, versioned metadata, `ArtifactReference` values, structure and provenance. Each `ArtifactReference` is a Generation-manifest relation/value, not a generic reference type for every derivative. | Controlled Product Data | Server metadata plus immutable Artifact custody | Retained according to release/reference/hold policy | `REQ-ID-002/004`, `REQ-WS-009` |
| Artifact Reference | A Controlled Product Data relation/value inside one Generation Manifest that pins one exact `ArtifactId`, digest and declared content role. Other owner Modules may retain their own exact `ArtifactId`/digest references inside BOM or Representation metadata; those references do not become Controlled Product Data `ArtifactReference` records. | Controlled Product Data | Stored only with the Generation Manifest and resolved through Artifact Custody | Retained with the manifest; provider/path changes never rewrite it | `REQ-ID-003/004`; `REQ-OPS-006` |
| Artifact | Provider-neutral logical identity for immutable binary content identified by digest. Product identity and manifests never depend on a filesystem path, bucket name or storage vendor. | Artifact Custody | Bytes are accessed only through Artifact Custody and an active Artifact Location; owner Modules retain exact identity/digest pins in their own metadata, while only a CPD Generation manifest uses the term `ArtifactReference` | Classification inherits the Logical Document/context; physical reclamation requires zero retained references | `REQ-FMT-001`; `REQ-OPS-003/006` |
| Artifact Location | Replaceable custody record that maps one Artifact digest to a Vault Endpoint, opaque provider key, verification/health state and replication/migration status. It is not part of document identity. | Artifact Custody | Selected only through policy; Client receives no opaque provider key or permanent credential | Several verified locations may coexist; retirement requires reconciliation and the applicable minimum-location policy | `REQ-OPS-003/004/006…008`; `QRS-012/014` |
| Vault Endpoint | Governed logical identity for one Artifact Gateway and its private Vault Adapter/location, with eligibility, health, locality, capacity and supported transfer capabilities. It is not a business Server or document folder. | Artifact Custody | Server control plane selects it; Gateway validates only grants addressed to that endpoint | Health/configuration history retained as operations evidence; provider secrets stay inside the Adapter | `REQ-WS-016`; `REQ-OPS-007/008`; ADR C1-011 |
| Artifact Transfer | Resumable upload/download session for one immutable candidate or pinned Artifact, with `TransferId`, `OperationId`, direction, selected endpoint, expected size/digest, accepted ranges/chunks and terminal state. | Artifact Custody; authoritative resource owner owns the later business decision | Client data plane uses an exact short-lived Transfer Grant; private staging is not visible as a Generation | Interrupted sessions may resume safely; abandoned candidates follow expiry/reconciliation and never become public by themselves | `REQ-WS-012/015/016`; `REQ-OPS-001`; `QRS-011/013` |
| Transfer Grant | Short-lived exact authorization for one transfer direction, OperationId/TransferId, candidate or Artifact, selected Vault Endpoint, expected size/digest, permitted ranges and expiry. It is not a login session or permanent store credential. | Artifact Custody | Issued through Server control plane; validated by addressed Gateway; cannot authorize another endpoint/object/direction | Expires/revokes under policy; retained only as bounded security/operation evidence without exposing secret material | `REQ-SEC-001/002`; `REQ-WS-016` |
| Transfer Receipt | Authenticated Gateway evidence of accepted ranges, size/digest verification and location result, correlated to one grant/transfer. It is not Check-in success. | Artifact Custody validates and retains the custody result; Gateway produces it | Server accepts only an exact valid receipt from the addressed Gateway; resource owner still revalidates and commits | Retain with operation/Audit for reconciliation; forged/mismatched/expired evidence fails closed | `REQ-SEC-002`; `REQ-WS-016`; `REQ-OPS-001` |
| Replication Task | Idempotent server-to-server request to create, repair or verify an Artifact Location from an existing verified source without another Client upload. | Artifact Custody | Operates only through Gateway/Vault Adapters; cannot change Generation/Release state | Retain source/target/digest/progress/outcome; failed target remains ineligible and source stays available | `REQ-OPS-007/008`; `QRS-014` |
| Storage/Durability Policy Version | Immutable activated rules for location eligibility, minimum verified locations by operation, failure-domain constraints, replication/repair and retirement. | Artifact Custody; Product Configuration Administration prepares governed versions | Exact policy version is evaluated before applicable Check-in/Release finalization; counts/thresholds remain `BLOCKED` until approved | Retain every version referenced by operation, location or Release evidence | `REQ-OPS-008`; `SPEC-OPEN-05`; `QRS-014` |
...
| BOM Representation | Non-authoritative Excel/PDF/CSV or other output from one exact Structure Snapshot and BOM View Profile, with output digest and producer provenance. Product Structure owns the Representation metadata, including its exact output `ArtifactId`/digest reference; this is not a CPD `ArtifactReference`. | Product Structure owns source/profile relationship, metadata and status; Artifact Custody holds immutable output bytes | Generated/served through controlled interfaces; only retained/referenced after Product Structure accepts metadata in its owner unit of work; never writable as authoritative structure | Retained when referenced by Release/history; otherwise governed derivative retention; `Current` only for its exact source/profile | `REQ-STR-005`, `REQ-LC-008` |
...
| Approval Policy / Decision | Exact policy identity/version, actor eligibility, required decision count/rule, `AllowSelfApproval` result, independent-approver requirement and attributable approval/rejection. The recorded self-approval result is evidence of the policy evaluation; it is not a Permission grant. | Lifecycle Governance | Server authority; read according to access policy | Retained with workflow/release evidence; the policy version is pinned to the Review Round and decision | `REQ-LC-003/004` |
...
| Representation | Preview or neutral derivative tied to one exact source Generation and source Artifact digest, with producing application/Adapter/tool versions and output digest. Format Intelligence retains its own exact source/output `ArtifactId`/digest pins and provenance; these are not CPD Generation-manifest `ArtifactReference` records. | Format Intelligence | Automatic application-assisted/standalone processing or attributable manual upload through one acceptance boundary; served through controlled interface | Non-authoritative; `Current` only for its pinned source, otherwise `Needs update`; may be regenerated, but history/provenance is retained | `REQ-FMT-005` |
...
**`DATA-VIEW-CORE-001` — Released-baseline identity (conceptual).** **Model profile:** UML-style ER
   domain view; `Draft 0.16`; product, data and quality reviewers. **Question:** which exact identities
...
erDiagram
    ...
    accDescr: An Operating Organization owns Logical Documents, each with Business Revisions and immutable Generations. Only a Generation manifest owns CPD ArtifactReferences; Product Structure and Format Intelligence may keep their own exact ArtifactId/digest pins in their metadata without becoming CPD ArtifactReferences. Generation manifests may pin Structure Snapshots. A Release Record pins exact Generations and approval evidence, plus zero or more exact Structure Pins only where its Release Policy selects or requires them, so later Working Heads cannot alter the released baseline.
...
**`DATA-VIEW-ART-001` — Immutable Artifact, scoped transfer and multi-location custody.**
**Model profile:** UML-style ER domain view; `Draft 0.17`; data, operations and security reviewers.
**Question:** how can direct resumable transfer and several Vault locations preserve one immutable
Artifact identity? **Scope:** Check-in upload transfer/grant/receipt, candidate, Generation manifest, Vault endpoint,
location and replication policy. **Excludes:** provider schema, cryptographic format, chunk
implementation, download transfer relationships and final Check-in gates. **Trace:** `REQ-WS-012/015/016`,
`REQ-OPS-001/003/004/006…008`, `VVP-017`, WS-08 and ST-01…07. **Legend:** crow's-foot marks logical
cardinality; a staged candidate and receipt remain private evidence until a committed Generation
references the verified Artifact.

```mermaid
erDiagram
    accTitle: Artifact transfer and multi-location custody identity
    accDescr: A Check-in Operation declares Artifact Transfers. Each transfer is authorized by one or more short-lived grants addressed to an eligible Vault Endpoint and produces authenticated receipts. Artifact Custody owns private staged candidates, verified logical Artifacts and one or more physical locations. Replication Tasks create or repair locations under an exact Storage Durability Policy version. Only a committed Controlled Product Data operation creates Generations whose manifests reference the logical Artifact.

    CHECKIN_OPERATION ||--o{ ARTIFACT_TRANSFER : coordinates
    ARTIFACT_TRANSFER ||--|{ TRANSFER_GRANT : authorized_by
    TRANSFER_GRANT }o--|| VAULT_ENDPOINT : addressed_to
    ARTIFACT_TRANSFER ||--o{ TRANSFER_RECEIPT : evidenced_by
    ...
    VAULT_ENDPOINT ||--o{ ARTIFACT_LOCATION : hosts
    TRANSFER_RECEIPT }o--o| ARTIFACT_LOCATION : verifies
    ARTIFACT ||--o{ REPLICATION_TASK : protects
    STORAGE_DURABILITY_POLICY_VERSION ||--o{ REPLICATION_TASK : governs
```

An upload remains a private staged candidate until the authoritative Check-in transaction publishes
the complete manifest. The Client receives only a short-lived Transfer Grant and moves bytes through
the selected Artifact Gateway; it never receives the Vault's opaque provider key or permanent
credential. Artifact Custody validates the Gateway's receipt, verifies/deduplicates the candidate and
allocates or reuses `ArtifactId`; Controlled Product Data subsequently records only a CPD
Generation-manifest `ArtifactReference`. `ArtifactId` and `ContentDigest` remain stable while
locations are replicated, repaired, selected, migrated or retired. Neither a completed transfer nor
a receipt is a Check-in result, and a replica is not a coordinated backup.
...
**Trace:** `REQ-ID-002…004`, `REQ-WS-001…016`, ADR C1-004/C1-005/C1-011, `WS-01…08`. **Legend:**
...
| Generation | Unique `GenerationId`; immutable after successful changed Check-in. | Belongs to exactly one Logical Document and Business Revision; Version within Revision starts at 1 and strictly increases for changed Check-ins. No Change creates neither a new Version nor a Generation. | No mutable lifecycle; becomes current Working Head by atomic Check-in. | Manifest pins Version, metadata schema, CPD Generation-manifest `ArtifactReference` values, structure and provenance. |
| Artifact | `ArtifactId` is provider-neutral; `ContentDigest` identifies immutable bytes. | One Generation may retain CPD Generation-manifest `ArtifactReference` values to multiple Artifacts; Product Structure/Format Intelligence may pin the same identity/digest in their own metadata without creating CPD records. | Artifact Custody verifies/deduplicates a private candidate before it can be referenced by validated owner commit; later provider migration changes no Generation. | Upload/validation/producer evidence, location history and digest read-check. |
| Artifact Location | Stable custody location identity points from one Artifact to one Vault Endpoint and opaque provider key. | It is not a Version, Generation or user-visible file identity; several locations may represent the same Artifact/digest. | Candidate → Verified → Degraded/Repairing → Retiring → Retired; applicable policy decides the required verified set. | Artifact Custody records health, copy verification, selection, failover, reconciliation and retirement evidence without exposing credentials. |
| Vault Endpoint | Stable endpoint identity describes one eligible Artifact Gateway/Vault Adapter and its governed locality/capability status. | It is independent of a document, physical path and business Server process. | Registered → Eligible → Degraded/Unavailable → Retired under operations/configuration authority. | Configuration, health and selection evidence are retained without provider secrets. |
| Artifact Transfer | `TransferId` is unique and correlated to one `OperationId`, direction, selected endpoint, expected Artifact/candidate and Actor/Workspace. | Chunks/ranges and receipts do not create document Versions; only a committed Check-in may publish a Generation. | Preparing → Transferring → Verified → Consumed, or Failed/Expired/NeedsReconciliation. | Artifact Custody retains accepted ranges, grant/receipt correlation, checksums, retry status and terminal result for safe resume without duplicate publication. |
| Transfer Grant / Receipt | Each grant/receipt has a unique identity and exact correlation to one transfer and endpoint. Secret grant material is not domain identity. | Grant renewal/replacement changes no document Version; receipt is custody evidence, not product state. | Grant: Issued → Used/Expired/Revoked. Receipt: Progress/Verified/Refused; only a valid verified result may support later owner commit. | Retain bounded claims, issuer/endpoint, expiry and result; omit reusable secret material. |
| Replication Task / Durability Policy | Task identity pins one Artifact/digest, source/target locations and exact policy version. | Copying or repairing bytes changes no Generation, Revision, Version or Release. | Planned → Copying → Verifying → Completed, or Failed/NeedsRepair; failed target remains ineligible. | Source/target, ranges, digest, policy, actor/system reason and result are auditable; exact thresholds remain open. |
...
| BOM Representation | Stable output identity with immutable bytes/digest and exact source/profile pins. Product Structure owns this metadata and its owner-specific output `ArtifactId`/digest reference; it is not a CPD Generation-manifest `ArtifactReference`. | Many outputs/formats may derive from one snapshot/profile; none changes the source Generation. | `Current` only after Product Structure accepts the metadata in its owner UoW; comparison with a newer source/profile yields `Needs update`. | Producer, time, source/profile, format, output digest and owner outcome retained. |
...
| `DATA-REL-004` | Generation Manifest contains CPD `ArtifactReference` relation/values to Artifacts by exact identity/digest and declared content role. | At least one primary Artifact where the format profile requires it; digest is immutable. | Missing/corrupt object is visible and blocks exact reproduction. | Manifest-to-object reconciliation |
...
| `DATA-REL-011` | Format Intelligence owns Representation metadata, source Generation/Artifact pins, its own exact output ArtifactId/digest pins, Format Capability Profile and producing application/Adapter/tool versions; manual upload also records attributable uploader and declared producer. These owner-specific references are not CPD Generation-manifest `ArtifactReference` records. Artifact Custody owns only immutable output bytes and verified identity/digest. | Many derivatives per Generation are allowed; none is authoritative until Format Intelligence accepts its metadata in the owner unit of work. `Current` is evaluated against the selected source Generation, never a floating latest reference; byte verification alone does not make it current. | Source/profile advance changes the older result to `Needs update`; conversion, byte-storage or metadata-acceptance failure preserves the source and authoritative product state. A stored candidate remains private/unreferenced until accepted. | CR-01…05 derivative provenance/staleness/policy tests |
...
| `DATA-REL-021` | Product Structure owns BOM Representation metadata, which pins the exact Structure Snapshot, BOM View Profile, producer/format and immutable output ArtifactId/digest in owner-specific fields; that pin is not a CPD `ArtifactReference`. | Many representations may exist; none is Product Structure authority. `Current` is evaluated against the selected source/profile and is assigned only after the Product Structure owner UoW commits the metadata. | Source/profile advance preserves prior output and yields `Needs update`; byte-storage success, mismatch or failed generation cannot change authoritative structure or create an accepted Representation. | BM-02/03 export and staleness tests |
| `DATA-REL-022` | A BOM Import Candidate pins its payload digest, exact base Structure Snapshot, proposed mapping and difference set; a named BOM Import coordinator accepts the confirmed candidate through one shared relational unit of work and links it to the resulting snapshot/Generation. | Candidate is non-authoritative; one successful confirmation maps to at most one atomic result. Product Structure writes only its Structure Snapshot; Controlled Product Data writes only its Generation/Working Head; each records its own Owner Command Outcome while Audit Evidence and applicable outbox share the coordinator UoW. | Malformed, unresolved, unauthorized, stale-base or faulted import rolls back all uncommitted authoritative writes, exposes no partial result and leaves the candidate safe for governed reconciliation. | BM-04/05 import tests |
...
| `DATA-REL-028` | An immutable Authorization Decision identifies server-established ActorContext, action, resource/expected state, evaluated membership and assignments, immutable Role Definition versions, Scope resolution and RBAC result. Access Policy obtains current IAM eligibility/security version through the read-only `IF-IAM-ELIGIBILITY-QUERY` port. | A granted RBAC result expresses eligibility only; it contains no business-gate or final owner result. The query port cannot re-enter an IAM mutation handler or hold an IAM mutation lock. | Evaluation failure or no grant fails closed. Explanation is safe for the requester's access; decision evidence cannot be used to retroactively reinterpret a prior outcome. | REQ-AUTH-006…008; REQ-AUD-001/002; RBAC-06/09/10 |
...
| `DATA-REL-030` | Artifact Custody owns one or more Artifact Locations behind its controlled Gateway/Vault Interface. | CPD Generation manifests pin Artifact identity/digest through `ArtifactReference`, never endpoint/provider/path. Product Structure and Format Intelligence may keep exact ArtifactId/digest values in their own metadata. Each retained Artifact satisfies the applicable minimum verified-location policy; several locations are ordinary, not migration-only. | Location failure/degradation triggers eligible-location selection and repair. No selection, replication, migration or retirement step changes a Generation, Version or Release Record. | `REQ-OPS-003/004/006…008`; `QRS-012/014`; storage-evolution tests |
| `DATA-REL-031` | An Artifact Transfer owns its accepted-range state and is authorized by exact short-lived Transfer Grants; authenticated Transfer Receipts report Gateway progress/verification for that same transfer and endpoint. | Direction, OperationId/TransferId, endpoint, candidate/Artifact, expected size/digest, permitted ranges and expiry must match. A receipt grants no product authority and is not Check-in success. | Wrong/altered/expired/replayed grant, forged/mismatched receipt or unavailable endpoint fails closed; safe resume may receive a renewed or replacement grant without changing operation identity. | `REQ-WS-012/015/016`; `REQ-SEC-001/002`; `QRS-011/013` |
...
| `DATA-REL-033` | An owner Command Outcome correlates one Authorization Decision to the resource owner's business-gate and final result. | The record belongs to the authoritative resource owner; it is not mutable Access Policy state or a generic cross-module transaction table. A named coordinator may retain multiple owners' outcomes only within the declared operation UoW, without owning their product state. | Commit-time authorization or owner-state revalidation failure produces a refused outcome and no authoritative business-state change; refusal evidence is committed separately where required. | `REQ-GOV-002`; `REQ-AUTH-008`; `REQ-AUD-001/002`; RBAC-10 |
| `DATA-REL-034` | A Replication Task creates, repairs or verifies an Artifact Location under one exact Storage/Durability Policy version. | Source location must already be verified; target becomes eligible only after complete size/digest verification and applicable failure-domain checks. Replication is server-to-server and requires no Client re-upload. | Interrupted/corrupt copy leaves target ineligible and source unchanged; policy shortfall blocks the applicable Check-in/Release but cannot be reported as data loss if another verified source remains. | `REQ-OPS-007/008`; `QRS-014`; ST-05…07 |
...
| Account/session operations | Authorized Account Administrator or signing-in user | Identity and Accounts | Provision/activate, sign-in/out, change/reset password, suspend/revoke and resolve eligibility. Material account/security mutation writes authoritative IAM state, its Owner Command Outcome, material Audit Evidence and applicable outbox in one owner UoW before success. Failed sign-in has no authoritative mutation but may be independently audited. | Typed bounded errors, no username enumeration through unauthenticated recovery responses; expired/reused proof refused; repeated admin actions do not create duplicate account identity. | No open signup; protected transport, Audit without secrets; current eligibility checked at protected requests and owner commit. |
...
| Effective-access evaluation | Authoritative product Module | Access Policy, which resolves Identity and Accounts plus Project Governance evidence itself | Owner requests authorization with server-established `ActorContext`, Permission, `ResourceId`, Scope and expected state. Access Policy reads current IAM eligibility/security version only through the read-only `IF-IAM-ELIGIBILITY-QUERY` port, then resolves Project/Group membership, Role Assignments, immutable Role Definition versions and Scope hierarchy and returns one immutable `AuthorizationDecision`. The query port cannot re-enter IAM mutation commands; Access Policy never returns the owner's business-gate or final command outcome. | Missing or unavailable eligibility evidence fails closed. The owner applies its own gates and requests commit-time revalidation before commit. | Correlation identifies contributing assignment/version IDs, the immutable decision and the separate owner outcome without leaking unauthorized data. |
...
| Workspace materialization control | Per-user Workspace through IDEA Server | Artifact Custody | Workspace requests an exact pinned Artifact/digest. Server authorizes the read; Artifact Custody selects an eligible verified location and returns the selected Gateway plus an exact short-lived read grant. | Resume uses the same TransferId/OperationId or a correlated replacement grant; changed authorization, unavailable location or stale evidence fails closed and returns a bounded recovery path. | Server-established ActorContext and owner authorization; no database/permanent Vault credential or opaque provider key is exposed. |
| Artifact byte download | Per-user Workspace | Selected Artifact Gateway/Vault Adapter | Workspace sends the read grant to the selected Gateway and receives only the permitted resumable byte ranges; it verifies the final expected digest locally. Business Server is not the payload proxy. | Interrupted multi-GB transfer resumes only missing verified ranges. Wrong endpoint/object/range, expiry, replay or digest mismatch remains not ready and cannot substitute another Artifact. | Gateway validates exact grant; receipt/progress is correlated and auditable; raw provider access is absent. |
...
| Check-in transfer preparation | Workspace through IDEA Server | Artifact Custody | One `CheckinOperationId` declares the confirmed scope, expected Generations, Reservations, manifests, sizes and digests. Artifact Custody selects an eligible Gateway/Vault and issues exact short-lived upload grants for changed candidates. | Changed inputs cannot reuse the operation; lost/expired grants may be replaced only after the same operation and current authorization/state are revalidated. | Server control plane records correlation; no permanent storage credential/path reaches the Client. |
| Check-in byte upload | Workspace | Selected Artifact Gateway/Vault Adapter | Workspace sends resumable checked ranges directly to the addressed Gateway. Gateway stores private candidate bytes, verifies chunks/size/full digest and returns an authenticated receipt; it cannot publish. | Repeating an accepted range is idempotent. Wrong/altered grant, corrupt/incomplete content or receipt mismatch fails closed. Abandoned candidates remain private and are expired/reconciled under policy. | Accepted ranges/checksums, endpoint, grant/receipt correlation and terminal custody outcome are auditable. |
| Check-in commit/status | Declared Check-in use-case coordinator | Named Controlled Product Data owner, Artifact Custody, Audit/outbox and Reservation records | Artifact Custody first validates receipts and applicable minimum-location policy. The coordinator then opens one shared relational unit of work and invokes only its declared owners; it owns no generic CRUD or product record. Controlled Product Data publishes the full logical Change Set, advances intended Working Heads, persists its `OwnerCommandOutcome` and marks every confirmed in-scope Reservation `Ended`; Audit evidence and transactional outbox commit atomically. `No Change` publishes no Generation but still ends its in-scope Reservation. | Transfer/receipt success is not commit success. Before-commit failure publishes none and ends none. The same `CheckinOperationId` returns the committed result, resumes safe missing work or reports an input conflict; uncertain response is resolved by status query before retry. | Server/IAM-established ActorContext, authorization, owner/Workspace/current Generation, durability policy and business gates are revalidated at commit. Audit distinguishes transferred, committed, failed and `NeedsReconciliation`; local work is never deleted by server failure. |
| Vault replication and repair | Artifact Custody | Source and target Artifact Gateway/Vault Adapters | One idempotent task copies an exact immutable Artifact by digest from a verified source to a target, verifies size/digest and records the resulting location under an exact policy version. Client does not upload again. | Interrupted/corrupt target remains ineligible; repair resumes safely; retirement occurs only after policy/reconciliation permits it. | Least-privilege task scope, source/target evidence, no document authority; replicas remain separate from coordinated backup. |
| Format analysis / Representation | Controlled Product Data | Isolated Format Intelligence Adapter or attributable manual-upload boundary | Immutable source Generation/Artifact digest plus exact Format Capability Profile. Artifact Custody verifies/stores output bytes and returns identity/digest; Format Intelligence then revalidates and accepts authoritative Representation metadata, including its owner-specific exact ArtifactId/digest pins, source pins and status in its owner UoW, with Owner Command Outcome, Audit Evidence and applicable outbox atomic to that acceptance. These pins are not CPD `ArtifactReference` records. | Retry by job identity; mismatched output is rejected; source advance yields `Needs update`; timeout/failure or metadata refusal cannot change source/product state. If metadata acceptance fails after byte storage, the candidate remains private/unreferenced for reconciliation. | Least-privilege one-job scope and bounded resources; release response comes from the versioned Release Policy. |
| BOM query/export | Web/Desktop or Release process | Product Structure / controlled Artifact custody | Product Structure resolves the exact Structure Snapshot and BOM View Profile, authorizes the export, generates a candidate, and asks Artifact Custody to verify/store immutable bytes. It then revalidates the exact source/profile/business conditions and accepts owner-specific BOM Representation metadata (RepresentationId, snapshot/profile pins, producer/format provenance, ArtifactId/digest, status) in a Product Structure owner UoW with its Owner Command Outcome, Audit Evidence and applicable outbox before COMMIT. The metadata pin is not a CPD `ArtifactReference`. | Query/export may retry by operation identity; no request follows floating latest where an exact result is required. If metadata acceptance fails after byte storage, the candidate remains private/unreferenced under reconciliation policy, authoritative structure is unchanged and the UI does not report a retained/Current Representation. | Owner authorization and commit-time revalidation; source/profile/output/actor/result, owner outcome and refusal/reconciliation evidence recorded; `REQ-STR-004/005`. |
| BOM import | Authorized Web/Desktop administration/work surface | Named BOM Import coordinator / Product Structure + Controlled Product Data | Payload and exact base snapshot create a non-authoritative candidate and validation/difference preview; separate confirmation invokes the named coordinator, which opens one shared relational UoW, revalidates authorization and expected state, asks Product Structure to write only its Structure Snapshot and Controlled Product Data to write only its Generation/Working Head, and records each owner outcome plus Audit Evidence/outbox atomically. | Invalid/stale/unauthorized/faulted candidate rolls back all uncommitted owner writes; no partial snapshot/Generation/Working Head is published; retry cannot create a duplicate result and the candidate remains safe for reconciliation. | Server-only authority; candidate digest, base, mapping, actor, confirmation, per-owner outcomes and coordinator result audited; `REQ-STR-006`. |
...
### 5.4 Multi-location custody, replication and provider evolution

Artifact Custody may maintain several verified locations before any provider migration. Adding a
Vault, repairing a replica or changing from a filesystem-backed Adapter to object storage or another
approved Adapter must not change a Logical Document, Revision, Version, Generation, Artifact identity
or Release Record. Replication/migration runs behind the controlled Gateway/Vault Interface and uses
the following sequence:

1. register the target Vault Endpoint/Adapter and keep it ineligible for ordinary resolution;
2. create an idempotent Replication Task from an already verified source location;
3. copy retained Artifacts by digest using scoped server-to-server transfer and resumable ranges;
4. verify full size/digest and create `Verified` Artifact Location records;
5. reconcile every retained manifest, hold, Release, durability-policy and backup/recovery-set reference;
6. enable selection from the target under the applicable health/locality/capacity policy while
   retaining the source location;
7. retire a source location only after the approved observation/rollback window, minimum-location
   policy and a second
   reconciliation pass.
...
minimum verified-location counts, failure domains, replication lag/capacity thresholds and migration
schedule remain Spec/Tech/operations decisions. Replicas improve byte custody but are not independent
backup evidence; the invariant above is the product/data contract required by `REQ-OPS-006…008` and
`QRS-012/014`.
...
carry that load. File-size distribution, growth rate, concurrency, Gateway/Vault topology, minimum
verified locations, failure domains, replication lag, transfer limits, retention and supported-format
limits remain open under `SPEC-OPEN-04/05`.
...
| PG3 design consistency | Ownership, relationships, semantic interfaces and failure behavior align with DOC-05@0.21 | Draft reconciliation authored; architecture/data/security review and exact successor Tech decision `NOT-RUN` |
...
| Workspace/scale and architecture correction | CHG covers Reservation, Reference, direct Gateway transfer, multi-location Vault custody, storage evolution, RBAC inheritance, tests, operations, release impact and ownership/security corrections | [IE-CHG-VAULT-XFER-001](registers/CHG-2026-09-17-multi-location-vault-transfer-architecture.md) records the latest successor; earlier CHG records remain historical; review/closure and runtime evidence remain open |
