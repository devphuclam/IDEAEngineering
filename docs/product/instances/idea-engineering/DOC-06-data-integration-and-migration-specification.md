...
> **Instance state**: controlled `Draft 0.16`. Sections 1–7 define the information and exchange
...
**Model profile:** UML-style ER domain view; `Draft 0.16`; data, operations and security reviewers.
...
erDiagram
    ...
    accDescr: A Check-in Operation declares Artifact Transfers, while Artifact Custody owns their private staged candidates, verified Artifact identities and replaceable locations. Only a committed Controlled Product Data operation creates Generations whose manifests own CPD ArtifactReferences; Product Structure and Format Intelligence may keep separate exact ArtifactId/digest pins in their own Representation metadata. Artifact Locations map each digest to replaceable storage providers without changing document or Generation identity.
...
`ArtifactId`; Controlled Product Data subsequently records only a CPD Generation-manifest
`ArtifactReference` in its Generation manifest. `ArtifactId` and `ContentDigest` remain stable when bytes move between storage
...
| PG3 design consistency | Ownership, relationships, semantic interfaces and failure behavior align with DOC-05@0.16 | Draft reconciliation authored; architecture/data/security review and Tech decision `NOT-RUN` |
...
| `DOWNSTREAM` | DOC-05 interfaces, DOC-07 increment, DOC-08 interactions, VVP and later implementation contracts | DOC-05@0.16 and DOC-08@0.12 Drafts authored and reconciled at source level; review `NOT-RUN` |
| `CHANGE` | CHG with data, interface, security, migration, recovery and release impact | Prior decisions remain traceable; [IE-CHG-WS-SCALE-001](registers/CHG-2026-09-10-workspace-transfer-storage-decisions.md) records the workspace/scale successor; [IE-CHG-ARCH-CORR-001](registers/CHG-2026-09-12-architecture-consistency-correction.md) is the predecessor and [IE-CHG-ARCH-CORR-002](registers/CHG-2026-09-12-architecture-consistency-correction-002.md) records the later transaction/ownership correction |
| `VERIFICATION` | Mapping/reconciliation, Workspace/Reference/transfer/storage, item/folder/copy, BOM, handoff, workflow, format, RBAC/delegation, authorization/import and restore procedures/results | Candidate `IE-VVP-CORE-001@0.16`; all product execution `NOT-RUN`; [IE-VEV-ARCH-CORR-003](registers/VEV-2026-09-12-architecture-consistency-correction-003.md) separately records the bounded source/rendition audit; VEV-001/002 remain historical evidence |
