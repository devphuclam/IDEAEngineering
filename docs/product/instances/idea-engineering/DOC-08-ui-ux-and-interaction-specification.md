...
> **Instance state**: controlled `Draft 0.7`. This document defines proposed user journeys,
...
| Source Links | [DOC-03](DOC-03-business-requirements.md), [DOC-04](DOC-04-software-requirements-specification.md), [current IDEA DDM workbench prototype](../../../../prototypes/idea-ddm-workbench.html), [administration prototype](../../../../prototypes/idea-ddm-administration.html), [retained earlier prototype](../../../../prototypes/controlled-document-workspace.html), [standards register](../../../governance/standards-register.md) |
...
| Change History | 0.7: separate Account Administration for users/groups/membership from PDM Administration for Access Policy, Workflow Roles and governed definitions; link the administration prototype as design evidence. 0.6: point to the current IDEA DDM workbench prototype and reconcile Feature/VVP pins; [IE-CHG-SOURCE-RECON-001](registers/CHG-2026-09-09-cross-document-reconciliation.md). 0.5 distinguished managed BOM view, pinned BOM Representation, independent parts-list document and previewed BOM import. 0.4 distinguished folder/placement, Rename and Create Copy. 0.3 showed one Version within Revision and the exact Generation. 0.2 added account/login/recovery tasks. |
...
| PDM Administrator | Configure document classes, metadata, numbering, Access Policy, Workflow Roles/workflow, locale and format profiles. | PDM Administration Web surface, separate from routine account and membership work. | See candidate/active versions, affected scope and difference before activation. | Administration prototype is design evidence only; representative administrator and usability evidence `UNKNOWN`. |
| Account Administrator | Manage Actors, IDEA Accounts, Login Identities, Business Groups and membership within delegated scope; assist recovery. | Account Administration Web surface. In normal operation this duty belongs to named System Management personnel. | Plain account/group status and consequences; no password display; never imply that account creation alone grants product access. | Project user may hold temporary bootstrap capability during development; production assignee, security policy and usability evidence remain unqualified. |
...
| `UX-JRN-009` Manage account, group or membership | Current actor holds Account Administration permission and an explicit delegated scope. | Select Accounts, Business Groups or Membership; identify stable Actor/group; preview add/remove/suspend/reset consequence; confirm. Account setup/reset uses the approved one-use channel. | Outcome is audited; existing passwords stay hidden; account creation alone grants no product access; out-of-scope/stale change is refused; last recovery path cannot be removed without a governed replacement. | `REQ-IAM-002/003/005/007`; `REQ-GOV-002`; `VVP-015` |
...
| `UX-JRN-013` Configure product access or workflow | Current actor holds the applicable PDM Administration permission. Existing group identities are selectable but membership is read-only on this surface. | Create a Draft candidate; choose document class/scope/state/action or Workflow Role/eligible group; validate and compare with the active version; activate through a separate confirmation. | Active policy/definition changes only after authorized activation and Audit. Invalid, stale, unresolved or self-authorizing candidate changes neither policy, workflow nor membership; running instances retain their pinned versions. | `REQ-GOV-002/005`; `REQ-LC-001/003/005`; AC-01…05; WF-01…06 |
...
| Administration application | Separate entry point with two permission-based work areas: Account Administration for Accounts/Business Groups/Membership, and PDM Administration for definitions/policy/workflow. A person assigned both permissions may see both areas after sign-in. | Navigation follows the administrative object being changed, not the engineer's document tree. Membership editing appears only in Account Administration; policy/workflow screens may select existing groups but cannot change their members. | `REQ-IAM-002/005`; `REQ-GOV-002/005`; DOC-05 interfaces | `Draft`; prototype evidence only |
...
| Account/group administration | Account Administrator views/changes an account, Business Group or membership. | Separate account/group controls from policy/workflow/document controls; confirmation identifies stable target, delegated scope and access consequence. Account creation and membership assignment are shown as separate actions. | Clear labels and keyboard-accessible confirmation, not color alone. | Denied, stale, out-of-scope or last-admin-protection result shows recovery guidance without exposing credentials or partially changing membership. | `REQ-IAM-002/005/007`; `REQ-GOV-002`; `UX-JRN-009` |
| Policy/workflow candidate | PDM Administrator edits a Draft Access Policy, Workflow or governed definition. | Show active/base version, candidate status, affected types/scopes/actions, referenced Business Groups and a readable difference before activation. Membership is read-only here. | Candidate editing and activation are separate keyboard-accessible actions; warnings are not confirmation. | Missing group/role/transition, stale base, invalid rule or lost authority refuses activation and leaves the active version/running instances unchanged. | `REQ-GOV-002/005`; `REQ-LC-001/003/005`; `UX-JRN-013` |
...
| Account Administration | List/detail and controlled operations for Actors, accounts, Login Identities, Business Groups and Membership. | Stable target/scope, expected version, permitted action and explicit confirmation. | No open signup, password read-back, policy editing or hidden product grant; membership and account activation are distinct; last-recovery-path safeguard. | `REQ-IAM-002/003/005/007`; `REQ-GOV-002`; `UX-JRN-009` | `Draft` |
| PDM Policy Administration | Prepare, validate, compare and activate Access Policy candidates that reference existing Business Groups. | Base/active/candidate versions, resource class/scope/state/action grants, group references and direct-grant exceptions. | Cannot create users/groups/membership; invalid/stale/self-authorizing candidate has no effect; activation is separate and audited. | `REQ-GOV-002/005`; `UX-JRN-013`; AC-01…05 | `Draft` |
| Workflow Administration | Prepare, validate, compare and activate Workflow/Approval definitions and Document-Class assignments. | Definition/base version, states, transitions, Workflow Roles, eligible-group references and decision rules. | Cannot create group membership; missing roles/transitions/groups or stale activation is refused; running instances keep pinned versions. | `REQ-LC-001/003/005`; `REQ-GOV-005`; `UX-JRN-013`; WF-01…06 | `Draft` |
...
The item-workspace HTML does not implement account administration, PDM policy/workflow administration
or BOM authoring/import. The separate administration HTML demonstrates only the proposed navigation
and object split; neither prototype is a conformance result or proof of Server authorization.
...
| Administration is represented only by a separate interactive prototype. | Web administration and applicable login surfaces | The prototype demonstrates the proposed split among accounts/groups, Access Policy and workflow, but it is not evidence of server authorization or evaluated task usability. | Principal Product Author; before the corresponding implementation. | UX-JRN-009/010/013; `idea-ddm-administration.html` | `OPEN`; design evaluation and implementation evidence absent |
...
| `CHANGE` | CHG covering interaction, accessibility, locale, risk, tests and release impact | Prior decisions remain traceable; [IE-CHG-SPEC-ARCH-QUALITY-001](registers/CHG-2026-09-10-spec-architecture-quality-baseline.md) records the current Administration interaction split and successor source |
| `VERIFICATION` | VVP/VEV identity, item-organization, BOM, manual, assistive, Unicode and cross-locale task evidence | `IE-VVP-CORE-001@0.11` authored; all execution `NOT-RUN` |
