...
| Approvers | Product Decision Authority approved the exact predecessor Tech baseline on 17-09-2026; approval of this 0.22 successor architecture is `NOT-RUN` |
...
| `IF-WORKFLOW-ADMIN` | Lifecycle Governance / Product Configuration Administration UI | Workflow/Approval definition candidate, Workflow Roles, RBAC eligibility rules, `AllowSelfApproval` and base version → validation/diff or activated version. | A definition cannot create accounts, Groups, memberships or Role Assignments. Missing roles/transitions/eligibility, unsafe policy conflict, stale base or self-authorizing adoption is refused; running instances remain pinned to prior versions. | `REQ-LC-001/003/005`, `REQ-GOV-005`, `REQ-AUTH-008`; WF-01…06; definition/version/actor/outcome audited. |
...
5. The seeded path requires one eligible independent approver and refuses self-approval/release.
   A later Approval Policy Version may explicitly enable self-approval, but only after RBAC and
   business-gate checks; another applicable policy requiring independence still blocks self-only
   approval, and the setting never grants Release. Missing actors or invalid configuration block the
   affected action with an explainable result.
