...
The Vault renderer requires a new evidence ID and refuses to write into an existing evidence
directory. Use it to create a successor record against the exact current sources; a later source
change needs another successor evidence record and output path.
